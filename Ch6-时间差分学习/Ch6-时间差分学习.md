...
##### 6.8 游戏，后状态(afterstates)和其他特例

本书尝试展示解决一大类任务的通用方法，但必然总会有一些例外。例如，通用的方法涉及学习一个行动-价值函数，但第1章展示的学习tic-tac-toe的TD方法学习了某种更像状态-价值函数的东西。若更仔细看这个例子就会发现，学得的函数在通常意义下既非行动价-值函数，也非状态-价值函数。惯常的状态-价值函数在有机会选择行为时评估状态，但在tic-tac-toe中使用的状态-价值函数在代理已经做出移动后才评估棋盘位置。我们称这些为**后状态(afterstates)**，在此之上的价值函数为**后状态价值函数(afterstates value function)**。当**知道环境动态初始部分知识而未必是全部动态时，后状态十分有用**。比如游戏时通常知道移动的即刻效果，知道每个棋子可能已定的结果位置，但却无从得知对手会如何应对。后状态是一种很自然的使用这种知识的方式，因此产生高效的学习方法。

设计关于后状态的算法更高效的原因在tic-tac-toe示例中十分明显。惯常的行动-价值函数会将位置和移动映射到价值的估计，但很多位置-移动对会产生同样的位置，比如：

<img src="note6_pics/tic-tac-toe.png" width="500px" text-align="middle" />

这种情况的位置-移动对不同但产生同样的“后状态”，因此必然有相同的价值。惯常的行动-价值函数会分别评估这两个对，而后状态价值函数会立即等价地评估这两个。任何关于左边位置-移动对的学习会立即迁移到右边的对。不仅仅是游戏，后状态还会出现在很多任务中。比如在排队任务中会有像分配顾客给服务员、回绝顾客或丢弃信息这样的行动，这种情况下行动实际上定义在它们的即使效果上，而这些都是完全已知的。

描述所有可能类型的特殊问题和相应特定的学习方法是不可能的，但本书形成的原则能广泛应用。比如，后状态方法也适于用广义策略迭代描述，使策略和（后）状态价值函数以本质相同的方式交互。在许多案例中依然面临on-policy和off-policy方法的选择以实现持续探索的需求。

**练习6.13**：描述如何将Jack的租车任务用后状态重构。在这个特定任务上，这样的重构会可能加速收敛吗？为何？